...
import { useGetAllProductCategoriesQuery } from '@/redux/features/categories/categoryManagement.api';
...
console.log(allCategories);
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      ...
      // ]
      setCategories(allCategories?.map(cat => ({ ...cat, icon: iconMap[(cat.icon)as keyof typeof iconMap] })))
    }
          ...
          {categories?.map((category) => (
            <Link key={category.slug} to={`/categories/${category.slug}`}>
              <div className="flex flex-col items-center justify-center p-6 bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 cursor-pointer">
                ...
                <span className="lg:text-lg font-semibold text-gray-800">{category.name}</span>
              </div>
            </Link>
          ))}
        </div>
